...
data = pd.read_csv(
    'data/jumlah_penduduk_pendidikan.csv', sep=',',
...
with st.container(border=True):
    ...
    with kolom3:
        pilihanjenis = data[(data['tahun'] == tahunterpilih) & (data['semester'] == semterpilih)]['jenis_pendidikan'].unique()
        jenisterpilih = st.selectbox("Filter Jenjang Pendidikan", pilihanjenis)
        
    if tahunterpilih and semterpilih and jenisterpilih:
        st.subheader(f"Sebaran Penduduk :green[Berpendidikan {jenisterpilih}] di Kota Bandung, :blue[Semester {semterpilih} Tahun {tahunterpilih}]")

        fig = px.choropleth_mapbox(
            data_frame=data[(data['tahun'] == tahunterpilih) & (data['semester'] == semterpilih) & (data['jenis_pendidikan'] == jenisterpilih)],
            ...
            hover_data=['kemendagri_nama_kecamatan', 'kemendagri_nama_desa_kelurahan', 'tahun', 'semester', 'jenis_pendidikan', 'jumlah_penduduk']
        )
...
st.subheader("", divider='rainbow')
st.link_button("Sumber Data", url="https://opendata.bandung.go.id/dataset/jumlah-penduduk-kota-bandung-berdasarkan-jenis-pendidikan")
