...
with st.container(border=True):
    ...
    with kolom3:
        pilihanjenis = data[(data['tahun'] == tahunterpilih) & (data['semester'] == semterpilih)]['jenis_kelamin'].unique()
        jenisterpilih = st.selectbox("Filter Jenis Kelamin", pilihanjenis)
        
    if tahunterpilih and semterpilih and jenisterpilih:
        st.subheader(f"Sebaran Penduduk :green[{jenisterpilih}] di Kota Bandung, :blue[Semester {semterpilih} Tahun {tahunterpilih}]")

        fig = px.choropleth_mapbox(
            data_frame=data[(data['tahun'] == tahunterpilih) & (data['semester'] == semterpilih) & (data['jenis_kelamin'] == jenisterpilih)],
            ...
            hover_data=['kemendagri_nama_kecamatan', 'kemendagri_nama_desa_kelurahan', 'tahun', 'semester', 'jenis_kelamin', 'jumlah_penduduk']
        )
