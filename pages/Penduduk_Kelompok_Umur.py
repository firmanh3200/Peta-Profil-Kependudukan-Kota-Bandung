...
data = pd.read_csv(
    'data/jumlah_kelompok_umur.csv', sep=',',
    dtype={'KODE_KEC':'str', 'kategori_umur':'str', 'jumlah_penduduk':'float'}
)
...
with st.container(border=True):
    ...
    with kolom3:
        pilihanjenis = data[(data['tahun'] == tahunterpilih) & (data['semester'] == semterpilih)]['kategori_umur'].unique()
    ...
    if tahunterpilih and semterpilih and jenisterpilih:
        ...
        fig = px.choropleth_mapbox(
            data_frame=data[(data['tahun'] == tahunterpilih) & (data['semester'] == semterpilih) & (data['kategori_umur'] == jenisterpilih)],
            ...
            hover_data=['kemendagri_nama_kecamatan', 'tahun', 'semester', 'kategori_umur', 'jumlah_penduduk']
        )
...
with st.container(border=True):
    filter_data = data[(data['tahun'] == tahunterpilih) & (data['semester'] == semterpilih) & (data['kategori_umur'] == jenisterpilih)]
    fig2 = px.sunburst(filter_data, path=['bps_nama_kabupaten_kota', 'kemendagri_nama_kecamatan'],
                      values='jumlah_penduduk',
                      hover_name='kategori_umur')
