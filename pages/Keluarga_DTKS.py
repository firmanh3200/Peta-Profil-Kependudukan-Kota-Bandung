...
data = pd.read_csv(
    'data/kk_dtks.csv', sep=',',
    dtype={'KODE_KD':'str', 'jumlah_keluarga':'float'}
)
...
with st.container(border=True):
    ...
    if tahunterpilih:
        st.subheader(f"Sebaran Keluarga di :green[Data Terpadu Kesejahteraan Sosial (DTKS)] Kota Bandung, :blue[Tahun {tahunterpilih}] (KK)")
        fig = px.choropleth_mapbox(
            data_frame=data[(data['tahun'] == tahunterpilih)],
            geojson=geojson_data,
            locations="KODE_KD",
            color="jumlah_keluarga",
            ...
            hover_data=['kemendagri_nama_kecamatan', 'tahun', 'jumlah_keluarga']
        )
...
with kol1:
    st.link_button("Sumber Data", url="https://opendata.bandung.go.id/dataset/jumlah-keluarga-di-data-terpadu-kesejahteraan-sosial-dtks-berdasarkan-kelurahan-di-kota-bandung")
with kol2:
