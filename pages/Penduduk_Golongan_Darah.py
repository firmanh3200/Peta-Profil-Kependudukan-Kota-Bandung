...
data = pd.read_csv(
    'data/penduduk_golongan_darah.csv', sep=',',
...
with st.container(border=True):
    ...
    with kolom3:
        pilihanjenis = data[(data['tahun'] == tahunterpilih) & (data['semester'] == semterpilih)]['tipe_goldar'].unique()
        jenisterpilih = st.selectbox("Filter Golongan Darah", pilihanjenis)
        
    if tahunterpilih and semterpilih and jenisterpilih:
        st.subheader(f"Sebaran Penduduk :green[ Bergolongan Darah {jenisterpilih}] di Kota Bandung, :blue[Semester {semterpilih} Tahun {tahunterpilih}]")

        fig = px.choropleth_mapbox(
            data_frame=data[(data['tahun'] == tahunterpilih) & (data['semester'] == semterpilih) & (data['tipe_goldar'] == jenisterpilih)],
            ...
            hover_data=['kemendagri_nama_kecamatan', 'kemendagri_nama_desa_kelurahan', 'tahun', 'semester', 'tipe_goldar', 'jumlah_penduduk']
        )
